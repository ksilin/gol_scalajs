...
def addWartRemoverToLintTarget = {
  ...
  inConfig(LintTarget) {
    wartremoverErrors ++= Warts.allBut(Wart.Any, Wart.NoNeedForMonad,
      Wart.NonUnitStatements, Wart.AsInstanceOf, Wart.Var, Wart.Throw) //Seq(
  ...
  //    wartremoverExcluded += baseDirectory.value / "src" / "main" / "scala" / "example" / "GoLApp.scala"
}
